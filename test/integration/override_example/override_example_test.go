...
func TestOverrideExample(t *testing.T) {
	t.Setenv("GCLOUD_TF_DOWNLOAD", "never")

	example := tft.NewTFBlueprintTest(t,
		tft.WithRetryableTerraformErrors(testutils.RetryableTransientErrors, 3, 2*time.Minute),
		tft.WithEnvVars(map[string]string{"GCLOUD_TF_DOWNLOAD": "never"}),
	)
	...
	example.DefineVerify(func(assert *assert.Assertions) {
		...
		downloaded := example.GetStringOutput("downloaded")

		assert.Equal("false", downloaded, "gcloud binary should not be downloaded when GCLOUD_TF_DOWNLOAD=never")
